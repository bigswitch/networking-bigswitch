...
class TestBigSwitchML2Router(test_l3.L3NatTestCaseBase,
    ...
    def test_router_add_interface_by_port_fails_nested(self):
        ...
        #             self.assertRaises(sql_exc.InvalidRequestError,
        ...
        #TODO(wolverineav): this fails for all the right reasons, but later
        # tanks in the after_create callback where we add to bsn_tenantpolicies
        # table. need to fix that properly
        pass
...
class TestBigSwitchMechDriverPortsV2(test_db_base_plugin_v2.TestPortsV2,
    ...
    def test_dont_bind_vnic_type_direct(self):
        ...
        with mock.patch(SERVER_POOL + '.rest_get_switch', return_value=True) \
                as rmock, \
                self.port(arg_list=(portbindings.HOST_ID,
                                    portbindings.VNIC_TYPE), **host_arg):
