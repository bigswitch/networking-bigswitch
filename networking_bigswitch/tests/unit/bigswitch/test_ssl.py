...
class TestSslSticky(test_ssl_certificate_base):
    ...
    def setUp(self):
        ...
        # ssl.create_default_context somehow is not present on zuul py27 job
        # patch to work around it
        if six.PY2:
            ssl.create_default_context = lambda _: 'fake_context'
    ...
    def test_sticky_cert(self):
        """Test certificate is stored

        :return:
        """
        # SSL connection should be successful and cert should be cached
        with mock.patch(HTTPSCON, autospec=True):
            self.network()
    ...
    def test_sticky_cert_using_cert_for_rest_call(self):
        """Test HTTPS Connection is used for REST Calls

        :return:
        """
        pl = directory.get_plugin()
        with mock.patch(HTTPSCON, autospec=True) as https_mock:
            pl.servers.rest_create_network('tenant', 'network')
