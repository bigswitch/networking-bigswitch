...
class ServerManagerTests(test_rp.BigSwitchProxyPluginV2TestCase):
    ...
    def test_retry_on_unavailable(self):
        ...
        with contextlib.nested(
            ...
            mock.patch(SERVERMANAGER + '.eventlet.sleep')
        ) as (srestmock, tmock):
...
class HashLockingTests(test_rp.BigSwitchProxyPluginV2TestCase):
    ...
    def test_hash_handle_lock_check_ts_previous_lock_exists(self):
        ...
        # 2nd thread came in just 10 seconds after first one, and first one
    ...
    def test_hash_handle_lock_no_check_ts_previous_lock_exists(self):
        handler1 = consistency_db.HashHandler()
        h1 = handler1.lock()
        self.assertTrue(h1)
        self.assertEqual(handler1.lock_marker,
    ...
    def test_hash_handle_lock_check_ts_previous_lock_not_expired(self):
        handler1 = consistency_db.HashHandler()
        h1 = handler1.lock()
        self.assertTrue(h1)
        self.assertEqual(handler1.lock_marker,
    ...
    def test_hash_handle_lock_check_ts_previous_lock_expired(self):
        handler1 = consistency_db.HashHandler()
        h1 = handler1.lock()
        self.assertTrue(h1)
        self.assertEqual(handler1.lock_marker,
        ...
        # thread 2 now tries to get lock with check_ts True
        # TOPO_SYNC_EXPIRED_SECS = 1800
        hh2_ts_under_limit = float(handler1.lock_ts) + 2000
        handler2 = consistency_db.HashHandler(hash_id='1',
                                              timestamp_ms=hh2_ts_under_limit)
        h2 = handler2.lock()
        self.assertTrue(h2)

    def test_hash_handle_lock_no_check_ts_previous_lock_not_expired(self):
        handler1 = consistency_db.HashHandler()
        h1 = handler1.lock()
        self.assertTrue(h1)
        self.assertEqual(handler1.lock_marker,
    ...
    def test_hash_handle_no_check_ts_lock_clash(self):
