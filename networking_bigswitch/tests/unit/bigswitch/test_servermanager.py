...
class ServerManagerTests(test_rp.BigSwitchProxyPluginV2TestCase):
    ...
    def test_no_sync_without_keystone(self):
        pl = directory.get_plugin()
        with\
            mock.patch(SERVER_UPDATE_TENANT_CACHE, return_value=(False)),\
    ...
    def test_no_send_all_data_without_keystone(self):
        pl = directory.get_plugin()
        with mock.patch(SERVER_UPDATE_TENANT_CACHE, return_value=(False)), \
            mock.patch(SERVER_TOPO_SYNC,
    ...
    def test_floating_calls(self):
        pl = directory.get_plugin()
        with mock.patch(SERVER_REST_ACTION) as ramock:
            body1 = {'id': 'somefloat'}
            body2 = {'name': 'myfl'}
