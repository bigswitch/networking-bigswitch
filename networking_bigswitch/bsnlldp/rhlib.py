...
def add_intf_to_map(intf_map, bridge_or_bond, type, intf_index):
    if bridge_or_bond not in intf_map:
        intf_map[bridge_or_bond] = {}
    if 'type' not in intf_map[bridge_or_bond]:
        intf_map[bridge_or_bond]['type'] = type
...
def get_network_interface_map():
            ...
            if config_type == 'ovs_bridge':
                ...
                for member in members:
                    ...
                    if member_type == 'interface':
                        intf_index = _get_intf_index(member.get('name'))
                        add_intf_to_map(
                            intf_map=intf_map, bridge_or_bond=bridge_name,
                            type='ovs_bridge', intf_index=intf_index)
                        break
                        ...
                        for nic in nics:
                            if nic.get('type') != 'interface':
                                continue
                            intf_index = _get_intf_index(nic.get('name'))
                            add_intf_to_map(
                                intf_map=intf_map, bridge_or_bond=bridge_name,
                                type='ovs_bridge', intf_index=intf_index)
                        break
                ...
                for nic in members:
                    ...
                    add_intf_to_map(
                        ...
                        type='linux_bond', intf_index=intf_index)
                ...
                for nic in members:
                    ...
                    if nic_type == 'ovs_dpdk_port':
                        ...
                        add_intf_to_map(
                            intf_map=intf_map, bridge_or_bond=bridge_name,
                            type='ovs_user_bridge', intf_index=intf_name)
                        break
    ...
    for bridge in intf_map:
        if intf_map[bridge]['type'] == 'ovs_user_bridge':
            ...
            continue
        if 'members' in intf_map[bridge]:
            intfs = []
            for index in intf_map[bridge]['members']:
                try:
                    idx = int(index)
                    if idx < intf_len:
