...
def read_file_as_str(path, identifier=None):
    """
    Read the file specified by path and part of path is specified by the
    identifier.

    Identifier may be empty if path is a complete absolute path.

    :param path: file path
    :param identifier: part of path specified as variable 'identifier'
...
def write_str_to_file(content, path, identifier=None):
    """
    Write the string content to the file specified by path and optional
    identifier.

    If identifier is empty, path must be complete and absolute.

    :param content: string content to be written to the file
    :param path: path of the file
    :param identifier: optional identifier in the path
    :return:
    """
    if identifier:
        path = path % identifier
...
def list_a_minus_list_b(list_a, list_b):
    """
    This method assumes input is two lists with unique elements aka sets. It
    then returns all unique elements in list_a, not present in list_b.
...
def find_pci_id(uplink):
    ...
    if os.path.exists(stash):
        return read_file_as_str(path=stash)
    ...
    write_str_to_file(content=pci_id, path=stash)
    return pci_id

def save_x710_intf_lldp_status(intf):
    """
    This will read the LLDP status being sent for the interface, only if it is
    an x710 network card.
    ...
    LOG.syslog("Uplink %s is PCI device %s", uplink, pci_id)

    if lldp_status[uplink]:
    ...
    vendor = int(read_file_as_str(path="/sys/bus/pci/devices/%s/vendor",
                                  identifier=pci_id), 16)
    device = int(read_file_as_str(path="/sys/bus/pci/devices/%s/device",
                                  identifier=pci_id), 16)
    ...
    if (vendor == 0x8086 and device == 0x1572):
        ...
        if os.path.exists("/sys/bus/pci/devices/%s/driver" % pci_id):
            ...
            if driver == X710_INTEL_DRIVER_STR:
                status = read_file_as_str(
                    path="/sys/kernel/debug/%s/%s/command",
                    identifier=(driver, pci_id))
                # update the status
...
def update_x710_lldp_status(pci_id, status):
    ...
    :return: None
    """
    write_str_to_file(content=status,
                      path="/sys/kernel/debug/%s/%s/command",
                      identifier=(X710_INTEL_DRIVER_STR, pci_id))

def save_and_stop_x710_intf_lldp(intf):
    """
    This checks if the interface is x710 interface, if yes, it will save the
    status in a local variable and stop LLDP on that interface.
