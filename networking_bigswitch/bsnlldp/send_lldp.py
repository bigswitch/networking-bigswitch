...
def is_redhat_dpdk(network_map):
    """Return a boolean status based on presence of linux_bond.

    presence of linux_bond signifies a DPDK deployment

    :param network_map:
    :return: bool
    """
    for br_or_bond in network_map:
        if network_map[br_or_bond]['type'] == 'linux_bond':
            return True
    return False
    ...
    for br_or_bond in network_map:
        ...
        if root_type == 'ovs_bridge':
            ...
            for member in network_map[br_or_bond]['members']:
                # get mac addr of intf
                # some cleaning is required here, string read from yaml is
                # unicode when converting to raw bytes, it throws error if
                # everything is not in ascii. all but intf_name is an issue
                intf_name = member
