...
class StateSyncer(object):
    ...
    def __init__(self, ks_auth_dict, nova_auth_dict):
        ...
        auth = v3.Password(auth_url=ks_auth_dict['auth_url'],
                           ...
                           project_domain_name=ks_auth_dict['project_domain_name'])
    ...
    def push_update(self):
        ...
        for hv in hypervisors:
            hv_list.append({
                ...
                'disk_capacity_gb': hv.local_gb,
                'disk_capacity_gb_used': hv.local_gb_used,
        ...
        for vm in vms:
            ...
            for addr in vm.addresses:
                for intf in vm.addresses[addr]:
                    interfaces.append({
                        'network_name': addr,
                        'mac_address': intf[bsn_consts.INTF_MAC_ADDR],
                        'ip_address': intf[bsn_consts.INTF_IP_ADDR],
                        'type': intf[bsn_consts.INTF_IP_TYPE],
        ...
        # log it for now
        LOG.info('json sent to BCF is %s' % post_data)
