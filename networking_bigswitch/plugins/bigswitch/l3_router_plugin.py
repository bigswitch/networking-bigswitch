...
class TransactionCache(dict):
    ...
    def add_transaction(self, tenant_id, transaction_id, obj_id):
        if tenant_id not in self.cache:
            self.cache[tenant_id] = {}
        self.cache[tenant_id][transaction_id] = obj_id

    def pop_transaction(self, tenant_id, transaction_id):
        if tenant_id not in self.cache:
            raise Exception(_('Tenant ID %(tenant_id)s not found in '
                              'transaction cache.') % {'tenant_id': tenant_id})
        if transaction_id not in self.cache[tenant_id]:
            raise Exception(_('Transaction ID %(txn_id)s for tenant '
                              '%(tenant_id)s not found in transaction cache.')
...
class L3RestProxy(cplugin.NeutronRestProxyV2Base,
    ...
    def router_before_create_callback(self, resource, event, trigger,
        ...
        self.txn_cache.add_transaction(tenant_id, router[BSN_TRANSACTION_ID],
                                       router['id'])
    ...
    def create_router(self, context, router):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                try:
                    actual_router_id = self.txn_cache.pop_transaction(
    ...
    def router_interface_before_create_callback(self, resource, event, trigger,
        ...
        self.txn_cache.add_transaction(
    ...
    def add_router_interface(self, context, router_id, interface_info):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                try:
                    actual_interface_id = self.txn_cache.pop_transaction(
                        tenant_id, bsn_transaction_id)
