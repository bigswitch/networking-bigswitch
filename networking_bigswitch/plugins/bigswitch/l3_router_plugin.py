...
class TransactionCache(dict):
    ...
    def __init__(self):
        self.cache = {}
        super(TransactionCache, self).__init__()

    def add_transaction(self, tenant_id, transaction_id):
        if tenant_id not in self.cache:
            self.cache[tenant_id] = {}
        self.cache[tenant_id][transaction_id] = None

    def update_transaction(self, tenant_id, transaction_id, obj_id):
        self.cache[tenant_id][transaction_id] = obj_id

    def read_and_remove_transaction(self, tenant_id, transaction_id):
        if tenant_id not in self.cache:
            raise Exception(_('Tenant ID %s not found in cache.'))
        if transaction_id not in self.cache[tenant_id]:
            raise Exception('BSN temp router_id not in router_create_cache.')

        del_id = self.cache[tenant_id].pop(transaction_id)

        if len(self.cache[tenant_id]) < 1:
...
class L3RestProxy(cplugin.NeutronRestProxyV2Base,
    ...
    def router_before_create_callback(self, resource, event, trigger,
        ...
        self.transaction_cache.update_transaction(
            tenant_id, router[BSN_TRANSACTION_ID], router['id'])
    ...
    def create_router(self, context, router):
        ...
        self.transaction_cache.add_transaction(tenant_id, bsn_transaction_id)
        try:
            ...
            with excutils.save_and_reraise_exception():
                try:
                    actual_router_id = (
                        self.transaction_cache.read_and_remove_transaction(
                            tenant_id, bsn_transaction_id))

                    self.servers.rest_delete_router(tenant_id,
                                                    actual_router_id)
    ...
    def router_interface_before_create_callback(self, resource, event, trigger,
        ...
        self.transaction_cache.update_transaction(
            tenant_id, interface_info[BSN_TRANSACTION_ID], subnet_id)
        with db.context_manager.reader.using(context):
            ...
            with excutils.save_and_reraise_exception():
                try:
                    ...
                    self.servers.rest_remove_router_interface(
                        tenant_id, router_id, actual_interface_id)
                except Exception as e:
                    LOG.error(_LE("Cannot clean up created object on BCF"
                                  " %(obj)s. Exception: %(exc)s"),
