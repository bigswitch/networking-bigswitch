...
class HashHandler(object):
    ...
    def _is_topo_sync_required(self, check_ts=True):
        ...
        if res:
            ...
            if timestamp_expired:
                LOG.debug("TOPO_SYNC: timestamp_expired "
                          "_is_topo_sync_required: True")
                return True, prev_ts
            elif not check_ts:
                # even if check_ts is False, we retrieve the prev_ts and return
                LOG.debug("TOPO_SYNC: check_ts is False "
                          "_is_topo_sync_required: True")
                return True, prev_ts
            else :
                LOG.debug("TOPO_SYNC: timestamp_expired is False, "
                          "check_ts is True _is_topo_sync_required: False")
                return False, prev_ts
        else:
            # no record found, sync required
            LOG.debug("TOPO_SYNC: no record found _is_topo_sync_required: "
                      "True")
            return True, '0'
    ...
    def _is_timestamp_expired(self, prev_ts='0'):
        ...
        if time_diff < TOPO_SYNC_EXPIRED_SECS:
            ...
            LOG.debug("TOPO_SYNC: _is_timestamp_expired: Current timestamp is "
                      "%(curr)s, previous timestamp is %(prev)s, "
                      "diff between the two is %(diff)s. "
                      "Diff < %(expired_secs)s, NOT EXPIRED.",
            ...
            if not current_lock_owner:
                ...
                if not self._optimistic_update_hash_record(res, new):
                    ...
                    LOG.debug(
                        ...
                        "Previous lock_ts: %(prev)s. Attempted update: %(new)s",
