        ...
        if resp[0] in ignore_codes:
            LOG.info(_LI("NeutronRestProxyV2: Received and ignored error "
                     ...
                     {'code': resp[2], 'action': action,
                      'resource': resource})
        elif cfg.CONF.RESTPROXY.auto_sync_on_failure:
            ...
            # start topo_sync in a separate thread and return current resp
            eventlet.spawn(self.force_topo_sync, **{'check_ts': False})
        return resp
