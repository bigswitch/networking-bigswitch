    ...
    try:
        ...
        try:
            ...
            LOG.warning(_LW("Config does not have property %(old_name)s "
                            "in group keystone_authtoken. Originally tried "
                            "and failed with %(name)s."),
...
class ServerPool(object):
    ...
    def __init__(self, timeout=False,
        ...
        LOG.debug("ServerPool: Performing initial topo_sync")
        self._force_topo_sync(check_ts=True)
    ...
    def rest_call(self, action, resource, data, headers, ignore_codes,
        ...
        for active_server in good_first:
            ...
            if ret[0] == httplib.NOT_FOUND:
                ...
                try:
                    self._force_topo_sync(check_ts=False)
                finally:
    ...
    def _force_topo_sync(self, check_ts=False):
        if not self.get_topo_function:
            raise cfg.Error(_('Server requires synchronization, '
                              'but no topology function was defined.'))
        ...
        LOG.debug("TOPO_SYNC: curr_ts is %s", curr_ts)
        # get lock on cdb
        hash_handler = cdb.HashHandler()
        res = hash_handler._get_current_record()
        if res:
            if 'LOCKED_BY' in res.hash:
                # TOPO_SYNC is currently in progress, return
                return True
            prev_ts = res.hash
        else:
            prev_ts = 0

        LOG.debug("TOPO_SYNC: prev_ts is %s", prev_ts)
        # if check_ts=True, execute only if diff > 1 hour
        time_diff = float(curr_ts) - float(prev_ts)
        if check_ts == True and time_diff < 3600:
            return True
        ...
        try:
            ...
            LOG.warning("TOPO_SYNC failed due to %s", e.message)
        finally:
