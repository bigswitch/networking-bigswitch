...
class ServerPool(object):
    ...
    def __init__(self, timeout=False,
        ...
        # update capabilities, quit if fails
        self.get_capabilities_or_die()
    ...
    def get_capabilities_or_die(self):
        """Throws an exception if no capabilities is returned by BCF

        This is to make it easier to mock in unit tests(so we don't have to
        mock the entire init function

        :return:
        """
        if not self.get_capabilities():
            raise cfg.Error(_('Failed to communicate with BCF Controller!'))
