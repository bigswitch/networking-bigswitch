...
class ServerPool(object):
    ...
    def _is_topo_sync_active(self, hash_handler=None):
        ...
        if res:
            ...
            if 'LOCKED_BY' in res.hash:
                ...
                time_diff = float(hash_handler.lock_ts) - float(prev_ts)
    ...
    def force_topo_sync(self, check_ts=True):
        ...
        if check_ts == True and time_diff < cdb.TOPO_SYNC_EXPIRED_SECS:
            LOG.info(_LI("TOPO_SYNC: Last topo_sync executed at %(prev_ts)s, "
                         "which is less than 30 mins ago. Skipping current "
                         "topo_sync."), {'prev_ts': prev_ts})
            return True
