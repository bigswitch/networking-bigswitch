...
repos.NeutronModules.MODULES['bsnstacklib'] = {'alembic-name':
                                               'bsn_service_plugin'}
...
def validate_bsn_service_plugin_installed():
    if not importutils.try_import('bsnstacklib'):
        alembic_util.err(_('Package bsnstacklib not installed'))


def get_script_location(neutron_config):
    location = '%s.db.migration:alembic_migrations'
    if neutron_config.service == 'bsn_service_plugin':
        validate_bsn_service_plugin_installed()
        base = 'bsnstacklib.plugins.bigswitch'
    elif neutron_config.service:
