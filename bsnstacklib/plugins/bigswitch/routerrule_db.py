...
class RouterRule_db_mixin(l3_db.L3_NAT_db_mixin):
    ...
    def _rule_a_superset_rule_b(self, rule_a, rule_b):
        ...
        if rule_a.action == rule_b.action == 'permit':
            if Set(rule_a.nexthops) != Set(rule_b.nexthops):
                return False
    ...
    def _min_priority(self, rules):
        """Get the minimum available rule_id. The difference between existing
        minimum and current ID is 5. If existing minimum is <= 5, returns False
        in the tuple.

        :param rules: list of existing rules with IDs
        :return tuple(boolean, min_id): boolean compaction required, min_id
        """
        min_priority = max_priority
        if len(rules) == 0:
        ...
        for rule in rules:
            min_priority = \
                rule.priority if rule.priority < min_priority else min_priority
    ...
    def _perform_compaction_and_update(self, context, router, rules):
        ...
        for new_rule_dict in new_rules_list:
            ...
            if new_rule_dict['priority'] == -1:
                ...
                old_rules_dict = self._make_router_rule_list(old_rules)
                new_rule = self._get_rule_diff(old_rules_dict, rules)
                LOG.debug('Insert rule with specified priority: %s' % new_rule)
                new_rule = RouterRule(
