...
class NeutronRestProxyV2Base(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _tenant_check_for_security_group(self, sg):
        sg['tenant_id'] = (SERVICE_TENANT if not sg['tenant_id']
                           else sg['tenant_id'])
    ...
    def _add_service_tenant_to_port(self, port):
        if not port.get('tenant_id'):
            port['tenant_id'] = SERVICE_TENANT
        if 'network' in port:
            if not port['network']['tenant_id']:
                port['network']['tenant_id'] = SERVICE_TENANT
