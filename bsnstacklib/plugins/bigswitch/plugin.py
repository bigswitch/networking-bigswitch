...
class NeutronRestProxyV2Base(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _map_port_hostid(self, port, network):
        ...
        if (portbindings.VIF_TYPE not in prepped_port or
            prepped_port[portbindings.VIF_TYPE] == portbindings.VIF_TYPE_UNBOUND):
            LOG.debug("Ignoring port notification to controller because of "
                      "UNBOUND vif_type.")
            return False
        ...
        if self._is_port_sriov(prepped_port):
            host_id = self._get_sriov_port_hostid(prepped_port, network)
            if not host_id:
                return False
            ...
            if vif_details:
                host_id = vif_details.get(pl_config.VIF_DET_BSN_VSWITCH_HOST_ID)
                if host_id:
                    prepped_port[portbindings.HOST_ID] = host_id
