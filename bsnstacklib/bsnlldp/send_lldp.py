...
def send_lldp():
    ...
    def _generate_senders_frames(intfs, chassisid, args):
        ...
        LOG.info("LLDP system-name is %s" % systemname)
        systemdesc = SYSTEM_DESC
        if args.system_desc:
            systemdesc = args.system_desc
        LOG.info("LLDP system-desc is %s" % systemdesc)
        for intf in intfs:
    ...
    if args.network_interface:
        ...
        try:
            ...
            intfs = []
    LOG.info("LLDP interfaces are %s" % ','.join(intfs))

    senders, frames = _generate_senders_frames(intfs, chassisid, args)
    interval = INTERVAL
    if args.interval:
        interval = args.interval
    LOG.info("LLDP interval is %d" % interval)
    while True:
